...
- **Dependencies and secrets**: Declare dependencies in `pyproject.toml`, which also holds the [deployment Python version](/langsmith/python/managed-deep-agents-deploy#set-the-python-version) under `[tool.mda]`. MDA loads `.env` locally and forwards non-reserved values as deployment secrets. Reserved platform variables and `.env` files are not included in the build archive. For more information, see [Deploy a Managed Deep Agent](/langsmith/python/managed-deep-agents-deploy).
