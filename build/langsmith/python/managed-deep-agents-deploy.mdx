...
Use a quoted `major.minor` string, with no patch version. MDA writes it to `python_version` in the generated `langgraph.json` and selects the matching [Agent Server](/langsmith/agent-server-overview) image. `requires-python` remains the project's own compatibility requirement. `mda init` scaffolds both fields, so generated projects start pinned to 3.14.
...
This controls the deployment image only. Local builds, `mda dev`, Harbor task images, and [execution sandboxes](/langsmith/python/managed-deep-agents-sandboxes) select their interpreters separately.
