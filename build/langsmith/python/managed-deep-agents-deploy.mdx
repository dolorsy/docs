...
MDA builds the deployment image on Python 3.11, 3.12, 3.13, or 3.14. Select the target in `pyproject.toml`:
...
`python-version` takes a quoted `major.minor` string, without a patch version or image suffix. MDA writes that value to `python_version` in the generated `langgraph.json` and selects the matching [Agent Server](/langsmith/agent-server-overview) image. `requires-python` stays the project's own compatibility requirement. `mda init` scaffolds both fields.

Without `python-version`, MDA selects the newest supported version that `requires-python` allows, or 3.14 when the project declares no `requires-python`. Set `python-version` explicitly to stay on the same minor version when MDA adds support for a newer Python.
