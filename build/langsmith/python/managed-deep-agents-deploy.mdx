...
Without `python-version`, MDA selects the newest supported version that `requires-python` allows. Set `python-version` explicitly to stay on the same minor version when MDA adds support for a newer Python.
