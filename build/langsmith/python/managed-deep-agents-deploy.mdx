...
MDA builds the deployment image on Python 3.11, 3.12, 3.13, or 3.14. By default it selects the newest of those that `requires-python` allows, so no configuration is required.

To pin a version instead, add the optional `[tool.mda]` table to `pyproject.toml`:
...
`python-version` takes a quoted `major.minor` string, without a patch version or image suffix. MDA writes that value to `python_version` in the generated `langgraph.json` and selects the matching [Agent Server](/langsmith/agent-server-overview) image. `requires-python` stays the project's own compatibility requirement. `mda init` scaffolds both fields, so generated projects start pinned to 3.14.

Set `python-version` explicitly to stay on the same minor version when MDA adds support for a newer Python.

To override the project setting for a single build, set `MDA_PYTHON_VERSION`:

```bash
MDA_PYTHON_VERSION=3.12 uv run mda deploy
```

MDA validates the selected target against `requires-python` and fails the build when the two contradict, because `pip install -e .` would fail in the built image. When compatibility depends on the image's exact patch version, which the image tag does not pin, MDA prints a warning and continues. The dependency install then checks the actual interpreter.

This setting controls the deployment image. Local build processing, `mda dev`, Harbor task images, and [execution sandboxes](/langsmith/python/managed-deep-agents-sandboxes) select their interpreters separately.
